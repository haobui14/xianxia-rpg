...
export const inventoryQueries = {
  ...
  async equipItem(runId: string, inventoryItemId: string, slot: string, itemData: any) {
    ...
    if (existing) {
      // Unequip first
      await supabase.from("character_equipment").delete().eq("id", existing.id);
    }
  ...
  async unequipItem(runId: string, slot: string) {
    ...
    await supabase.from("character_equipment").delete().eq("id", equipped.id);
...
export const leaderboardQueries = {
  ...
  async getLeaderboard(limit: number = 100) {
    // Use imported supabase
    const { data, error } = await supabase.from("leaderboard_view").select("*").limit(limit);
  ...
  async updateRelationship(
    ...
    if (existing) {
      ...
      const newLevel = Math.max(
        -100,
        Math.min(100, existing.relationship_level + relationshipChange)
      );
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          interaction_history: interactionData
