...
export function getSpiritRootBonus(grade: SpiritRootGrade): number {
  switch (grade) {
    case 'ThiênPhẩm': return 2.0;
    case 'Hiếm': return 1.5;
    case 'Khá': return 1.2;
    case 'PhổThông': return 1.0;
  }
}
...
export function useConsumableItem(state: GameState, itemId: string): boolean {
  ...
  if (item.effects) {
    ...
    if (item.effects.permanent_perception) state.attrs.perception += item.effects.permanent_perception as number;
