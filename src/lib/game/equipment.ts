...
export function calculateTotalMaxStats(state: GameState): Partial<CharacterStats> {
  ...
  Object.values(state.equipped_items).forEach(item => {
    if (item?.bonus_stats) {
      ...
      if (item.bonus_stats.stamina) bonuses.stamina_max = (bonuses.stamina_max || 0) + item.bonus_stats.stamina;
    }
  });
 ...
 */
export function getEquipmentBonus(state: GameState, stat: keyof CharacterAttributes | 'hp' | 'qi' | 'stamina' | 'cultivation_speed'): number {
  let bonus = 0;
