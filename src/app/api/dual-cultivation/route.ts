  ...
  try {
    ...
    if (action === 'toggle') {
      ...
      if (state.progress.cultivation_path === 'dual') {
        // Disable dual mode - keep body progress but stop splitting exp
        updatedProgress.cultivation_path = 'qi';
        updatedProgress.exp_split = 100;
      } else {
        // Enable dual mode
    ...
    return NextResponse.json({
      ...
      message: action === 'toggle'
        ? (updatedProgress.cultivation_path === 'dual' ? 'Dual cultivation enabled' : 'Dual cultivation disabled')
