    ...
    if (action === 'equip') {
      ...
      if (currentEquipped) {
        ...
        if (existingInInventory) {
          ...
          state.inventory.items.push({ ...currentEquipped, is_equipped: false, quantity: 1 });
        }
      }

      // Equip the new item (always quantity 1 when equipped)
      state.equipped_items[item.equipment_slot] = { ...item, is_equipped: true, quantity: 1 };
      ...
      if (equippedItem) {
        ...
        if (existingItem) {
          ...
          state.inventory.items.push({ ...equippedItem, is_equipped: false, quantity: 1 });
        }
