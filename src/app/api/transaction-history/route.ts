...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalSpentSilver = purchases.reduce(
      (sum: number, t: any) => sum + (t.price_silver || 0),
      0
    );
    const totalSpentStones = purchases.reduce(
      (sum: number, t: any) => sum + (t.price_spirit_stones || 0),
      0
    );
    const totalEarnedSilver = sales.reduce((sum: number, t: any) => sum + (t.price_silver || 0), 0);
    const totalEarnedStones = sales.reduce(
      (sum: number, t: any) => sum + (t.price_spirit_stones || 0),
      0
    );
