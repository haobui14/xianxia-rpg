...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalDamageDealt = history.reduce((sum: number, h: any) => sum + (h.player_damage_dealt || 0), 0);
    const totalDamageTaken = history.reduce((sum: number, h: any) => sum + (h.player_damage_taken || 0), 0);
    ...
    return NextResponse.json(
      { error: 'Failed to load combat history' },
      { status: 500 }
    );
  }
}
