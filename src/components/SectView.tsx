...
export default function SectView({ state, locale }: SectViewProps) {
  ...
  const { sect, rank, contribution, reputation, joined_date, missions_completed, mentor, mentor_en, benefits } = sect_membership!;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-xianxia-dark border border-xianxia-accent/30 rounded-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              <span className="text-gray-400">{locale === 'vi' ? 'Danh tiếng:' : 'Reputation:'}</span>
              <span className="text-green-400">{reputation}/100</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">{locale === 'vi' ? 'Ngày gia nhập:' : 'Joined:'}</span>
        ...
        <div className="bg-xianxia-dark border border-xianxia-accent/30 rounded-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-400">{locale === 'vi' ? 'Điểm cống hiến:' : 'Contribution Points:'}</span>
              <span className="text-yellow-400 font-bold">{contribution}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">{locale === 'vi' ? 'Nhiệm vụ hoàn thành:' : 'Missions Completed:'}</span>
      ...
      <div className="bg-xianxia-dark border border-xianxia-accent/30 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className={`p-4 rounded-lg border ${benefits.cultivation_bonus > 0 ? 'border-green-500/50 bg-green-900/20' : 'border-gray-600 bg-gray-800/20'}`}>
            <div className="flex items-center justify-between">
              ...
              <span className={`font-bold ${benefits.cultivation_bonus > 0 ? 'text-green-300' : 'text-gray-500'}`}>
                {benefits.cultivation_bonus > 0 ? `+${benefits.cultivation_bonus}%` : locale === 'vi' ? 'Không có' : 'None'}
              </span>
            </div>
          </div>

          <div className={`p-4 rounded-lg border ${benefits.resource_access ? 'border-blue-500/50 bg-blue-900/20' : 'border-gray-600 bg-gray-800/20'}`}>
            <div className="flex items-center justify-between">
              <span className={benefits.resource_access ? 'text-blue-400' : 'text-gray-400'}>
                💎 {locale === 'vi' ? 'Kho tài nguyên' : 'Resource Access'}
              </span>
              <span className={`font-bold ${benefits.resource_access ? 'text-green-300' : 'text-red-300'}`}>
                {benefits.resource_access ? (locale === 'vi' ? 'Có' : 'Yes') : (locale === 'vi' ? 'Không' : 'No')}
              </span>
            </div>
          </div>

          <div className={`p-4 rounded-lg border ${benefits.technique_access ? 'border-purple-500/50 bg-purple-900/20' : 'border-gray-600 bg-gray-800/20'}`}>
            <div className="flex items-center justify-between">
              <span className={benefits.technique_access ? 'text-purple-400' : 'text-gray-400'}>
                📖 {locale === 'vi' ? 'Tàng kinh các' : 'Technique Library'}
              </span>
              <span className={`font-bold ${benefits.technique_access ? 'text-green-300' : 'text-red-300'}`}>
                {benefits.technique_access ? (locale === 'vi' ? 'Có' : 'Yes') : (locale === 'vi' ? 'Không' : 'No')}
              </span>
            </div>
          </div>

          <div className={`p-4 rounded-lg border ${benefits.protection ? 'border-red-500/50 bg-red-900/20' : 'border-gray-600 bg-gray-800/20'}`}>
            <div className="flex items-center justify-between">
              <span className={benefits.protection ? 'text-red-400' : 'text-gray-400'}>
                🛡️ {locale === 'vi' ? 'Sự bảo vệ' : 'Sect Protection'}
              </span>
              <span className={`font-bold ${benefits.protection ? 'text-green-300' : 'text-red-300'}`}>
                {benefits.protection ? (locale === 'vi' ? 'Có' : 'Yes') : (locale === 'vi' ? 'Không' : 'No')}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-xianxia-dark border border-xianxia-accent/30 rounded-lg p-6">
        ...
        <div className="flex items-center justify-between text-sm">
          <div className={`text-center ${rank === 'NgoạiMôn' ? 'text-xianxia-accent font-bold' : 'text-gray-500'}`}>
            {locale === 'vi' ? 'Ngoại Môn' : 'Outer'}
          </div>
          <div className="flex-1 h-1 bg-gray-700 mx-2">
            <div className={`h-full bg-xianxia-accent transition-all ${rank !== 'NgoạiMôn' ? 'w-full' : 'w-0'}`}></div>
          </div>
          <div className={`text-center ${rank === 'NộiMôn' ? 'text-xianxia-accent font-bold' : rank === 'NgoạiMôn' ? 'text-gray-500' : 'text-gray-400'}`}>
            {locale === 'vi' ? 'Nội Môn' : 'Inner'}
          </div>
          <div className="flex-1 h-1 bg-gray-700 mx-2">
            <div className={`h-full bg-xianxia-accent transition-all ${['ChânTruyền', 'TrưởngLão', 'ChưởngMôn'].includes(rank) ? 'w-full' : 'w-0'}`}></div>
          </div>
          <div className={`text-center ${rank === 'ChânTruyền' ? 'text-xianxia-accent font-bold' : ['NgoạiMôn', 'NộiMôn'].includes(rank) ? 'text-gray-500' : 'text-gray-400'}`}>
            {locale === 'vi' ? 'Chân Truyền' : 'True'}
          </div>
          <div className="flex-1 h-1 bg-gray-700 mx-2">
            <div className={`h-full bg-xianxia-accent transition-all ${['TrưởngLão', 'ChưởngMôn'].includes(rank) ? 'w-full' : 'w-0'}`}></div>
          </div>
          <div className={`text-center ${rank === 'TrưởngLão' ? 'text-xianxia-accent font-bold' : rank === 'ChưởngMôn' ? 'text-gray-400' : 'text-gray-500'}`}>
            {locale === 'vi' ? 'Trưởng Lão' : 'Elder'}
          </div>
          <div className="flex-1 h-1 bg-gray-700 mx-2">
            <div className={`h-full bg-xianxia-accent transition-all ${rank === 'ChưởngMôn' ? 'w-full' : 'w-0'}`}></div>
          </div>
          <div className={`text-center ${rank === 'ChưởngMôn' ? 'text-xianxia-accent font-bold' : 'text-gray-500'}`}>
